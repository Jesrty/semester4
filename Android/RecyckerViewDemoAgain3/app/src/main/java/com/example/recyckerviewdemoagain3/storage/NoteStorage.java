package com.example.recyckerviewdemoagain3.storage;

import androidx.annotation.NonNull;

import com.example.recyckerviewdemoagain3.MainActivity;
...
public class NoteStorage {
    ...
    private static void getInitialNotes(){
        ...
        addOneTimeQuery(db.collection(path).orderBy("head").limit(queryLimit));
    }

    public static void getNextNotes() {
        addOneTimeQuery(db.collection(path).orderBy("head").startAfter(lastVisible).limit(queryLimit));
    }
            ...
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if (task.isSuccessful()){
                    ...
                    if (task.getResult().size() > queryLimit){
                        mainActivity.setIsLastItemReahed(true);
                    }
                }
            }
        });
